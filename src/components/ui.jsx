...
export function Card({ className = '', children }) {
  return (
    <div className={`rounded-2xl glass-card border border-white/10 ${className}`}>
      {children}
    </div>
  )
}
...
export function Modal({ open, onClose, title, children, footer }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="absolute inset-0 bg-black/50 backdrop-blur-sm anim-fade" onClick={onClose} />
      <div className="relative w-full sm:max-w-md rounded-t-3xl sm:rounded-2xl glass-strong border border-white/15 shadow-2xl max-h-[92dvh] flex flex-col anim-rise">
...
export const inputCls =
  'w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-slate-100 placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-[#f6d353]/40 focus:border-[#f6d353]/60'
    ...
    <Modal
      ...
      footer={
        <div className="flex gap-2 justify-end">
          <button onClick={onClose} className="rounded-xl px-4 py-2 text-sm font-medium border border-white/12 text-slate-300 hover:bg-white/10">Cancelar</button>
...
export const btnGhost =
  'rounded-xl px-4 py-2.5 text-sm font-semibold border border-white/12 text-slate-200 hover:bg-white/10 active:scale-[0.98] transition disabled:opacity-50 disabled:pointer-events-none'
